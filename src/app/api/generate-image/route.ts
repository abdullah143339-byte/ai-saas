...
export async function POST(request: Request) {
  try {
    ...
    if (imageData) {
      ...
      url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?model=flux-realism&width=${Math.min(w || 1024, 1024)}&height=${Math.min(h || 1024, 1024)}&img_input=${imgParam}`;
    } else {
      url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?model=flux-realism&width=${w || 1024}&height=${h || 1024}`;
    }
